...
class ProductItemDetails extends Component {
  ...
  addToCart = () => {
    ...
    if (cartList === null) {
      const objectList = JSON.stringify([{...productData, quantity}])

      localStorage.setItem('cart_list', objectList)
    } else {
      const parsedList = JSON.parse(cartList)
      const IndexValue = parsedList.findIndex(item => item.id === id)
      if (IndexValue === -1) {
        const newItem = {...productData, quantity}
